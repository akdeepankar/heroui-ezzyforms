...
const FormsContent = ({ activeFormTab: initialActiveFormTab = "all" }) => {
  ...
  const filteredForms = mockForms
    .filter((form) =>
      activeFormTab === "all" ? true : form.status === activeFormTab,
    )
    .filter((form) =>
  ...
  const getUniqueValues = (key) =>
    Array.from(new Set(dummyResponses.map((r) => r[key])));

  // For pagination, filter and slice the rows:
  const filteredRows = dummyResponses.filter(
    (row) =>
      (responseFilters.name === "" ||
        row.name.toLowerCase().includes(responseFilters.name.toLowerCase())) &&
      (responseFilters.email === "" ||
        row.email
          .toLowerCase()
          .includes(responseFilters.email.toLowerCase())) &&
      (responseFilters.q1.length === 0 ||
        responseFilters.q1.includes(row.q1)) &&
      (responseFilters.q2.length === 0 ||
        responseFilters.q2.includes(row.q2)) &&
      (responseFilters.q3 === "" ||
        row.q3.toLowerCase().includes(responseFilters.q3.toLowerCase())),
  );
  const totalPages = Math.ceil(filteredRows.length / rowsPerPage);
  const paginatedRows = filteredRows.slice(
    (responsePage - 1) * rowsPerPage,
    responsePage * rowsPerPage,
  );
      ...
      {selectedFormForResponses ? (
        <div className="w-full h-full min-h-[60vh]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <Card className="bg-white border border-gray-100 rounded-xl flex flex-col items-center justify-center p-6">
              ...
              <div className="text-2xl font-bold text-gray-900">
                {selectedFormForResponses.responses}
              </div>
            </Card>
            <Card className="bg-white border border-gray-100 rounded-xl flex flex-col items-center justify-center p-6">
              ...
              <div className="text-2xl font-bold text-gray-900">
                {selectedFormForResponses.completionRate}%
              </div>
            </Card>
            <Card className="bg-white border border-gray-100 rounded-xl flex flex-col items-center justify-center p-6">
              ...
              <div className="text-lg font-bold text-gray-900">
                {selectedFormForResponses.lastUpdated}
              </div>
            </Card>
            <Card className="bg-white border border-gray-100 rounded-xl flex flex-col items-center justify-center p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <Tabs
              className=""
              selectedKey={responsesTab}
              variant="solid"
              onSelectionChange={setResponsesTab}
            >
          ...
          {responsesTab === "table" ? (
            <div className="w-full bg-white border border-green-200 rounded-xl">
                ...
                <table className="w-full border border-green-200 text-[14px] font-sans bg-transparent">
                  <thead className="sticky top-0 z-20">
                    <tr className="border-b border-green-100 text-sm font-semibold text-gray-800 bg-white">
                      <th className="border border-green-100 px-2 py-2">
                        Name
                      </th>
                      <th className="border border-green-100 px-2 py-2">
                        Email
                    ...
                    <tr
                      className="sticky top-[38px] z-10"
                  ...
                  <tbody>
                    {paginatedRows.map((row, idx) => (
                      <tr
                        key={idx}
                        className={`${idx % 2 === 0 ? "bg-white" : "bg-[#f8fafb]"} border-b border-green-100 hover:bg-[#eaf6e6] transition-colors`}
                      >
              ...
              <div className="flex justify-end items-center gap-2 mt-3 p-4">
                ...
                <Button
                  disabled={responsePage === totalPages || totalPages === 0}
                  size="sm"
                  variant="flat"
                  onClick={() =>
                    setResponsePage((p) => Math.min(totalPages, p + 1))
                  }
                >
                  Next
                </Button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card className="bg-white border border-gray-100 rounded-xl p-6 flex flex-col items-center">
                ...
                <ResponsiveContainer height={220} width="100%">
                  <BarChart
                    data={Object.entries(
                      dummyResponses.reduce((acc, r) => {
                        acc[r.q1] = (acc[r.q1] || 0) + 1;
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-900">
                    {mockForms.filter((f) => f.status === "active").length}
                  </p>
                </div>
              </div>
              <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 flex items-center">
                  ...
                  <p className="text-xl font-bold text-gray-900">
                    {Math.round(
                      mockForms
                        ...
                        .reduce((sum, form) => sum + form.completionRate, 0) /
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg">
                  ...
                  >
                    All Forms
                    <span
                      className={`ml-1 text-xs ${activeFormTab === "all" ? "bg-indigo-100" : "bg-gray-200"} px-1.5 py-0.5 rounded-full`}
                    >
                      {mockForms.length}
                    </span>
                  </Button>
                  ...
                  >
                    <Icon
                      className={`mr-1 ${activeFormTab === "active" ? "text-indigo-600" : ""}`}
                      icon="solar:check-circle-bold"
                      width={16}
                    />
                    Active
                    <span
                      className={`ml-1 text-xs ${activeFormTab === "active" ? "bg-indigo-100" : "bg-gray-200"} px-1.5 py-0.5 rounded-full`}
                    >
                      {mockForms.filter((f) => f.status === "active").length}
                    </span>
                  </Button>
                  ...
                  >
                    <Icon
                      className={`mr-1 ${activeFormTab === "draft" ? "text-indigo-600" : ""}`}
                      icon="solar:pen-bold"
                      width={16}
                    />
                    Drafts
                    <span
                      className={`ml-1 text-xs ${activeFormTab === "draft" ? "bg-indigo-100" : "bg-gray-200"} px-1.5 py-0.5 rounded-full`}
                    >
                ...
                <div className="flex gap-2 w-full md:w-auto">
                  ...
                  <Dropdown>
                    <DropdownTrigger>
                      <Button
                        startContent={
                          <Icon icon="solar:sort-by-time-linear" width={16} />
                        }
                        variant="bordered"
                      >
                        Sort
                    ...
                    >
                      ...
                      <DropdownItem key="most-responses">
                        Most Responses
                      </DropdownItem>
              ...
              {sortedForms.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 px-4">
                  ...
                  <p className="text-gray-500 text-center mb-6">
                    {searchQuery
                      ...
                      : "You don't have any forms yet in this category."}
                  </p>
                  <Button
                    color="primary"
                    startContent={<Icon icon="solar:add-circle-bold" />}
                ...
                <Table aria-label="Forms table" className="border-collapse">
                  <TableHeader>
                    ...
                    <TableColumn className="text-xs uppercase text-center">
                      RESPONSES
                    </TableColumn>
                    <TableColumn className="text-xs uppercase text-center">
                      COMPLETION RATE
                    ...
                    <TableColumn className="text-xs uppercase text-right">
                      ACTIONS
                    </TableColumn>
                  </TableHeader>
                  <TableBody>
                      ...
                      >
                        <TableCell>
                          <div className="flex items-center">
                            <div
                              className={`w-10 h-10 rounded-lg bg-${form.color}-100 flex items-center justify-center mr-3`}
                            >
                              <Icon
                                className={`text-${form.color}-500`}
                                icon={form.icon}
                                width={20}
                              />
                            </div>
                        ...
                        <TableCell>
                          <div className="flex items-center">
                            <span
                              className={`inline-block rounded-full h-2.5 w-2.5 mr-2 ${form.status === "active" ? "bg-green-500" : "bg-yellow-500"}`}
                            />
                      ...
                      .map((form) => (
                        <div key={form.id} className="relative pt-1">
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center">
                              <div
                                className={`w-6 h-6 rounded-md bg-${form.color}-100 flex items-center justify-center mr-2`}
                              >
                                <Icon
                                  className={`text-${form.color}-500`}
                                  icon={form.icon}
                          ...
                          <div className="overflow-hidden h-2 mb-2 text-xs flex rounded-full bg-gray-200">
                            <div
                              ...
                              style={{
                                width: `${(form.responses / Math.max(...mockForms.map((f) => f.responses))) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
